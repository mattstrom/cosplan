// Rankings tab: tier each of your picks (must-see / want / if time).

import { el, personDot } from './dom.js';
import { TIERS } from '../logic.js';
...
export function renderRankings(ctx) {
  ...
  const sections = [...byDay.entries()].map(([day, dayPicks]) =>
    el('section', {},
      ...
      dayPicks.map(({ event, tier }) => el('div', { class: 'card rank-row' },
  ...
  return el('div', {}, personChips, sections,
    el('p', { class: 'hint' }, 'Tiers feed the conflict resolver: Must-see = 3 points, Want = 2, If time = 1.'));
}
